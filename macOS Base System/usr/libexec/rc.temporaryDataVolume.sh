...
if [ -f "/etc/rc.diag.cdrom" ]; then
	...
	# Executes tasks that require the Preboot and Data volume setup in their established state
	/usr/libexec/bs-data-tasks
	set +ex
else
	...
	# Since the data "volume" is now established, execute tasks that require it.
	/usr/libexec/bs-data-tasks
fi
